...
if [ "${PLATFORM_TYPE}" == "Linux" ]; then
    ...
    if [ "${ENVIRONMENT_NAME}" == "prod" ]; then
        ...
        if [ "${PHP_FPM_STATUS}" != "php${PHP_VERSION}-fpm" ]; then
            ...
            local addPackageList="php${PHP_VERSION}-common php${PHP_VERSION}-cli php${PHP_VERSION}-curl php${PHP_VERSION}-fpm php${PHP_VERSION}-gd php${PHP_VERSION}-mbstring php${PHP_VERSION}-intl php${PHP_VERSION}-uuid php${PHP_VERSION}-xml php${PHP_VERSION}-zip php${PHP_VERSION}-bcmath"
    ...
    if [ "${ENVIRONMENT_NAME}" == "prod" ]; then
        ...
        if [ -d "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}" ]; then
            # >>>> PHP-FPM - php.ini
            if [ -f "/etc/php/${PHP_VERSION}/fpm/php.ini" ]; then
                sudo cp -fv "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}/fpm/php.ini" "/etc/php/${PHP_VERSION}/fpm/php.ini"
            fi

            # >>>> PHP-FPM - php-fpm.conf
            if [ -f "/etc/php/${PHP_VERSION}/fpm/php-fpm.conf" ]; then
                sudo cp -fv "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}/fpm/php-fpm.conf" "/etc/php/${PHP_VERSION}/fpm/php-fpm.conf"
            fi

            # >>>> PHP-FPM - pool.d/www.conf
            if [ -f "/etc/php/${PHP_VERSION}/fpm/pool.d/www.conf" ]; then
                sudo cp -fv "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}/fpm/pool.d/www.conf" "/etc/php/${PHP_VERSION}/fpm/pool.d/www.conf"
            fi

            # >>>> PHP-FPM - conf.d/10-opcache.ini
            if [ -f "/etc/php/${PHP_VERSION}/fpm/conf.d/10-opcache.ini" ]; then
                sudo cp -fv "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}/fpm/conf.d/10-opcache.ini" "/etc/php/${PHP_VERSION}/fpm/conf.d/10-opcache.ini"
            fi
        ...
        if [ -d "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}" ]; then
            ...
            if [ -f "/etc/php/${PHP_VERSION}/cli/php.ini" ]; then
                sudo cp -fv "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}/cli/php.ini" "/etc/php/${PHP_VERSION}/cli/php.ini"
            fi

            # >>>> PHP - conf.d/20-xdebug.ini
            if [ -f "/etc/php/${PHP_VERSION}/cli/conf.d/20-xdebug.ini" ]; then
                sudo cp -fv "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}/cli/conf.d/20-xdebug.ini" "/etc/php/${PHP_VERSION}/cli/conf.d/20-xdebug.ini"
            fi
        else
            echo "There is not the folder: ${PROJECT_PATH}"
            echo
  ...
  # --------------------------------------------------------------------------------------------------------------------
    ...
    if [ "${ENVIRONMENT_NAME}" == "prod" ]; then
        ...
        if [ -d "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}" ]; then
            # >>>> PHP-FPM - php.ini
            if [ -f "/etc/php/${PHP_VERSION}/fpm/php.ini" ]; then
                sudo cp -fv "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}/fpm/php.ini" "/opt/homebrew/etc/php/${PHP_VERSION}/fpm/php.ini"
            fi
        ...
        if [ -d "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}" ]; then
            # >>>> PHP - php.ini
            if [ -f "/etc/php/${PHP_VERSION}/cli/php.ini" ]; then
                sudo cp -fv "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}/cli/php.ini" "/opt/homebrew/etc/php/${PHP_VERSION}/php.ini"
            fi

            # >>>> PHP - conf.d/20-xdebug.ini
            if [ -f "/etc/php/${PHP_VERSION}/cli/conf.d/20-xdebug.ini" ]; then
                sudo cp -fv "${PROJECT_PATH}/scripts/console/app/php/${PHP_VERSION}/cli/conf.d/20-xdebug.ini" "/opt/homebrew/etc/php/${PHP_VERSION}/conf.d/20-xdebug.ini"
            fi
        else
            echo "There is not the folder: ${PROJECT_PATH}"
            echo
